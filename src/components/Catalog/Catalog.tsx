...
export function Catalog({ catalog, className, showExpandAll = true }: CatalogProps) {
   ...
   */
  useEffect(() => {
    if (activeIndex === null || !catalogRef.current || !itemRefs.current[activeIndex]) {
      return
    }
    ...
    // 计算激活项相对于容器的位置
    const itemRelativeTop = itemRect.top - containerRect.top + catalogContainer.scrollTop

    // 计算目标滚动位置：让激活项显示在容器中间
    const targetScrollTop = itemRelativeTop - containerRect.height / 2 + itemRect.height / 2
